...
for i = 2:nsamples
    ...
    %
    [sorted_distance_array, indices] = sort(distances);
    for j = 1:min(length(sorted_distance_array), k)
        if (LocalPlanner(x, samples(:, indices(j))))
            nedges = nedges + 1;
            edges(nedges, :) = [i, indices(j)];
            edge_lengths(nedges) = distances(indices(j));
        end
    end
